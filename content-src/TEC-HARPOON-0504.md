---
id: TEC-HARPOON-0504
slug: harpoon-implant
title: "Harpoon Implant — якорный боевой трос"
zone: "05_TECHNOLOGIES"
category: "technologies"
status: "published"
lang: "ru"
version: "v1.0"
image: "/assets/content/placeholder.png"
tags:
  - harpoon
  - implant
  - mobility
  - striker
preview_kicker: "ZONE · 05_TECHNOLOGIES · STATUS: PUBLISHED"
preview_logline: "Тактический тросовый имплант для вертикальных рывков и штурма."
preview_markers:
  - "Проект Striker: городская война"
  - "Кинетический якорь и автонаведение"
  - "Риск перегрева и сбоев"
preview_signature:
  - "Палитра: сталь/неон"
  - "Режим: tether assault"
  - "Статус: published"
---
<style>
.ax-demo {
  --accent: #39c6ff;
  --accent-soft: rgba(57, 198, 255, 0.2);
  background: linear-gradient(135deg, rgba(10, 12, 16, 0.98), rgba(12, 18, 26, 0.92));
  border: 1px solid rgba(255, 255, 255, 0.08);
  border-radius: 18px;
  padding: 28px;
  box-shadow: 0 12px 40px rgba(0, 0, 0, 0.35);
}
.ax-demo * { box-sizing: border-box; }
.ax-demo-hero { display: grid; gap: 8px; margin-bottom: 16px; }
.ax-demo-kicker { font-size: 12px; letter-spacing: 0.24em; text-transform: uppercase; color: var(--accent); }
.ax-demo-hero h1 { margin: 0; font-size: 28px; }
.ax-demo-sub { margin: 0; color: #c6ccdc; font-size: 15px; }
.ax-demo-teaser { margin-bottom: 16px; color: #e6e9f3; }
.ax-demo-teaser p { margin: 0 0 10px; }
.ax-demo-grid { display: grid; gap: 12px; grid-template-columns: repeat(auto-fit, minmax(180px, 1fr)); }
.ax-demo-card { background: rgba(8, 12, 18, 0.6); border: 1px solid rgba(255, 255, 255, 0.08); padding: 12px 14px; border-radius: 12px; }
.ax-demo-card h3 { margin: 0 0 6px; font-size: 14px; text-transform: uppercase; letter-spacing: 0.08em; color: var(--accent); }
.ax-demo-card p { margin: 0; color: #d6d9e6; }
.ax-demo-signature { margin: 18px 0; padding: 14px 16px; border-left: 3px solid var(--accent); background: rgba(255, 255, 255, 0.04); color: #dfe3f2; }
.ax-demo-gallery { display: grid; gap: 12px; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); margin-bottom: 16px; }
.ax-demo-gallery figure { margin: 0; }
.ax-demo-gallery img { width: 100%; border-radius: 12px; display: block; }
.ax-demo-gallery figcaption { font-size: 12px; color: #b6bccd; margin-top: 6px; }
.ax-demo-related h3 { margin: 0 0 8px; font-size: 14px; text-transform: uppercase; letter-spacing: 0.1em; color: var(--accent); }
.ax-demo-related ul { margin: 0; padding-left: 18px; color: #d5d9e5; }
.ax-demo-related li { margin-bottom: 6px; }
</style>

<section class="ax-demo" style="--accent: #39c6ff; --accent-soft: rgba(57, 198, 255, 0.2);">
  <header class="ax-demo-hero">
    <div class="ax-demo-kicker">ZONE · 05_TECHNOLOGIES · STATUS: PUBLISHED</div>
    <h1>Harpoon Implant — якорный боевой трос</h1>
    <p class="ax-demo-sub">Тросовый имплант для штурма, рывков и вертикальной мобильности.</p>
  </header>

  <section class="ax-demo-teaser">
    <p>Проект Striker создавался для городского боя: бойцы должны были цепляться за стены, перехватывать этажи и уходить с линии огня за секунды.</p>
    <p>Имплант запускает титановый трос с автонаведением и регулирует натяжение под скорость рывка. Технология мощная, но капризная.</p>
  </section>

  <section class="ax-demo-facts">
    <div class="ax-demo-grid">
      <article class="ax-demo-card">
        <h3>Функция</h3>
        <p>Кинетический якорь и быстрые вертикальные перемещения.</p>
      </article>
      <article class="ax-demo-card">
        <h3>Режимы</h3>
        <p>Штурмовые рывки, удержание позиции, ближний удар тросом.</p>
      </article>
      <article class="ax-demo-card">
        <h3>Стабильность</h3>
        <p>Риск самосрабатывания на ранних этапах адаптации.</p>
      </article>
      <article class="ax-demo-card">
        <h3>Ограничения</h3>
        <p>Дистанция до 15 м, перегрев при серийных выстрелах.</p>
      </article>
    </div>
  </section>

  <blockquote class="ax-demo-signature">«Трос — это не путь. Это удар по высоте.»</blockquote>

  <section class="ax-demo-gallery">
    <figure>
      <img src="/assets/content/placeholder.png" alt="Harpoon implant schematic" loading="lazy" />
      <figcaption>Контуры кинетической системы закрепления.</figcaption>
    </figure>
  </section>

  <section class="ax-demo-related">
    <h3>Related</h3>
    <ul>
      <li>Neural Scanner — тактический прогноз</li>
      <li>Magnetic Implant — сцепление и грави-контроль</li>
      <li>Liza — первый активный носитель</li>
      <li>Endeavor — разработчик протокола</li>
    </ul>
  </section>
</section>
