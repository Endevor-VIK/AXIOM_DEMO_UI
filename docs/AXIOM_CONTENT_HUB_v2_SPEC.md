...
2. Все действия выполняются в актуальной ветке, указанной заказчиком  
   (на момент подготовки ТЗ: `feat/content-v2.1-fix/content-hub-redesign`).
...
- [ ] A.1.1. Создан каталог `content-src/` в корне репозитория.
- [ ] A.1.2. Для каждого логического файла контента (AXIOM, VIKTOR и др.) определён уникальный `id` (формат: `NN.NN_NAME`).
- [ ] A.1.3. Для каждого файла создан соответствующий Markdown-файл в `content-src/` с корректным `front-matter`.
- [ ] A.1.4. Проверено, что все обязательные поля `front-matter` присутствуют (`id`, `title`, `zone`, `category`, `status`, `lang`, `version`, `image`, `tags`, `preview_*`).
- [ ] A.1.5. Содержимое Markdown после `---` не содержит артефактов разметки, не относящихся к контенту (например, старые HTML-обёртки).

#### A.2. Каталог `public/content-html`

- [ ] A.2.1. Создан каталог `public/content-html/`.
- [ ] A.2.2. Для каждого `id` из `content-src/*.md` существует файл `public/content-html/<id>.html`.
- [ ] A.2.3. Внутри `*.html` содержится только тело, корректно вставляемое в `<article class="axr-body">`.
- [ ] A.2.4. Проверено, что HTML не содержит дублирующих `<html>`, `<head>`, `<body>`.

#### A.3. Индекс `content-index.json`

- [ ] A.3.1. Создан каталог `src/features/content/data/`.
- [ ] A.3.2. Создан файл `src/features/content/data/content-index.json`.
- [ ] A.3.3. Для каждого `id` из `content-src/*.md` имеется запись в `content-index.json`.
- [ ] A.3.4. Поля JSON соответствуют схеме, описанной в Части 1 (раздел 2.3).
- [ ] A.3.5. Проверено, что `id` в JSON совпадает с именами файлов HTML и `id` из `front-matter`.
...
- [ ] B.1.1. Создан каталог `scripts/` в корне репозитория.
- [ ] B.1.2. Создан файл `scripts/build-content.ts` **или** `scripts/build-content.js`.
- [ ] B.1.3. Скрипт обрабатывает все файлы `content-src/*.md`.
- [ ] B.1.4. Скрипт для каждого файла формирует HTML в `public/content-html/`.
- [ ] B.1.5. Скрипт полностью перезаписывает `content-index.json` актуальными данными.
- [ ] B.1.6. При ошибке структуры `front-matter` скрипт завершает работу с понятным сообщением (какой файл, что не так).

#### B.2. Зависимости

- [ ] B.2.1. В `package.json` добавлены необходимые библиотеки для парсинга Markdown/`front-matter`.
- [ ] B.2.2. Все добавленные пакеты помещены в `devDependencies`, если они используются только на этапе сборки.
- [ ] B.2.3. В логе зафиксированы названия и версии добавленных пакетов.

#### B.3. Интеграция в build-процесс

- [ ] B.3.1. В `package.json` добавлен скрипт `build:content`.
- [ ] B.3.2. Основной скрипт `build` вызывает `build:content` **до** запуска сборки Vite.
- [ ] B.3.3. Локально успешно выполнена команда `npm run build:content` (или аналог) без ошибок.
- [ ] B.3.4. Локально успешно выполнена команда полного `build` проекта.
...
- [ ] C.1.1. Создан каталог `src/features/content/components/`.
- [ ] C.1.2. Создан каталог `src/features/content/pages/`.
- [ ] C.1.3. Все новые компоненты CONTENT HUB находятся внутри этих каталогов.

#### C.2. Компонент `ContentSidebar`

- [ ] C.2.1. Создан файл `src/features/content/components/ContentSidebar.tsx`.
- [ ] C.2.2. Компонент принимает на вход список записей и `selectedId`.
- [ ] C.2.3. Реализован callback `onSelect(id)`.
- [ ] C.2.4. Активный элемент визуально подсвечивается по классу (аналог `axr-menu-item--active`).
- [ ] C.2.5. Реализовано поле поиска по `id`, `title` и `tags`.
- [ ] C.2.6. Стиль соответствует Red Protocol и не ломает текущую общую тему.

#### C.3. Компонент `ContentPreview`

- [ ] C.3.1. Создан файл `src/features/content/components/ContentPreview.tsx`.
- [ ] C.3.2. Вёрстка основана на референсах `ax-design/preview/*.html`.
- [ ] C.3.3. Все значения (заголовок, логлайн, списки, теги, путь к изображению) приходят через пропсы и не зашиты жёстко.
- [ ] C.3.4. Кнопка `Open source` вызывает переданный коллбек или переход к READER.
- [ ] C.3.5. Адаптивность: на узких экранах сплит превращается в вертикальный стек.

#### C.4. Страница `ContentHubPage`

- [ ] C.4.1. Создан файл `src/features/content/pages/ContentHubPage.tsx`.
- [ ] C.4.2. Страница импортирует `content-index.json` и формирует список контента.
- [ ] C.4.3. При первой загрузке выбирается корректный `selectedId` (по параметру в урле или по первому элементу списка).
- [ ] C.4.4. Страница отображает слева `ContentSidebar`, справа — `ContentPreview`.
- [ ] C.4.5. Кнопка `Open source` на превью открывает READER для соответствующего `id`.
- [ ] C.4.6. Маршрут CONTENT (существующий) корректно рендерит `ContentHubPage`.
...
- [ ] D.1.1. Создан файл `src/features/content/pages/ReaderPage.tsx`.
- [ ] D.1.2. Вёрстка страницы соответствует прототипу `ax-design/reader/READER_VIEW.html`.
- [ ] D.1.3. Реализована верхняя шапка (кнопка `← Content`, блок `id + версия + статус`, гамбургер).
- [ ] D.1.4. Реализовано левое меню, которое появляется поверх контента под шапкой.
- [ ] D.1.5. Убедиться, что левое меню **не перекрывает** шапку и начинается строго под ней (как зафиксировано в последней версии CSS).

#### D.2. Загрузка HTML-содержимого

- [ ] D.2.1. Страница по `id` формирует путь `"/content-html/<id>.html"`.
- [ ] D.2.2. Реализована загрузка HTML (через `fetch` или импорты, в зависимости от архитектуры проекта).
- [ ] D.2.3. При успешной загрузке HTML вставляется в `<article class="axr-body">`.
- [ ] D.2.4. Обработаны состояния:
  - загрузка (лоадер/скелетон),
  - ошибка (понятное сообщение с `id` и кнопкой «Назад»).

#### D.3. Маршрутизация `/content/:id`

- [ ] D.3.1. В системе роутинга добавлен маршрут вида `/content/:id` (или эквивалентный).
- [ ] D.3.2. Маршрут рендерит `ReaderPage`.
- [ ] D.3.3. При отсутствии записи в `content-index.json` по `id` страница показывает 404-экран для контента.
- [ ] D.3.4. Кнопка `← Content` возвращает пользователя на страницу CONTENT HUB.

#### D.4. Связь с CONTENT HUB

- [ ] D.4.1. Кнопка `Open source` в превью корректно открывает нужный `id` в READER.
- [ ] D.4.2. Переход назад из READER возвращает на CONTENT HUB с тем же выбранным `id` (насколько это возможно в рамках текущей системы роутинга/истории).
- [ ] D.4.3. Проверено, что при ручном вводе урла `/content/<id>` страница работает корректно и не ломает приложение.
...
- [ ] E.1.1. Найдены все старые компоненты/страницы, связанные с CONTENT.
- [ ] E.1.2. Список найденных файлов и путей зафиксирован в `AXIOM_CONTENT_HUB_v2_LOG.md`.
...
- [ ] E.3.1. Найдены CSS/SCSS файлы/фрагменты, относящиеся к старому CONTENT.
- [ ] E.3.2. Мёртвые стили удалены или интегрированы в новые компоненты.
