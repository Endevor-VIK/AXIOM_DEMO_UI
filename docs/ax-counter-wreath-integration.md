...
* [x] Старые круги и «ореолы» убраны / не влияют на венки. *(градиенты и тени перенесены в `styles/app.css` и `styles/counter-wreath.css` в рамках текущего прохода)*
* [x] `CounterWreath` подключён на HOME, сетка ровная, без наложений. *(новая сетка `repeat(3, minmax(200px, 1fr))` выровняла три венка в ряд)*
* [ ] Два красных кольца читаются; плитки поверх, с редким наслаиванием. *(тон красных колец уже ближе к референсу, но нужен финальный тюн по насыщенности и контрасту)*
* [ ] Плавные анимации на hover/click; реверс на mouseleave; без фонового rAF. *(основной цикл уже останавливается, требуется доделать угловой стаггер и финальный реверс)*
* [x] Нет TS-ошибок/варнингов в `wreath.ts`. *(добавлены проверки `prefers-reduced-motion`, контекст канваса и очистка слушателей)*
* [ ] A11y: `role`, `aria-label`, `aria-live`, клавиатура. *(частично: `aria-live` и `role` есть, нужно завершить поддержку клавиатурного фокуса для обёрток-ссылок)*
* [ ] 60 fps в профайлере, `prefers-reduced-motion` соблюдён. *(motion query внедрён, но профилирование FPS ещё не проведено)*
