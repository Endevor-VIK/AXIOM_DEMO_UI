...
* [x] Два красных кольца читаются; плитки поверх, с редким наслаиванием. *(толщины колец откалиброваны, плитки клампятся между inner/outer радиусами)*
* [x] Плавные анимации на hover/click; реверс на mouseleave; без фонового rAF. *(стаггер по углу пересчитан в миллисекунды, реверс синхронизирован, фоновых rAF нет)*
* [x] Нет TS-ошибок/варнингов в wreath.ts. *(включены guard'ы для prefers-reduced-motion и очистка слушателей)*
* [x] A11y: role, aria-label, aria-live, клавиатура. *(canvas помечен presentation/aria-hidden, корневые дивы получают aria-label, RouteWreath обеспечивает клавиатурный доступ)*
* [ ] 60 fps в профайлере, prefers-reduced-motion соблюдён. *(оптимизации внедрены, требуется подтвердить профайлером при следующем прогоне)*
* [x] Компонент легко переиспользуется на маршрутах AUDIT/CONTENT/NEWS. *(добавлены RouteWreath блоки на AUDIT, CONTENT, NEWS поверх CounterWreath)*
...
- ⚠️ *Расхождение:* референс использует заметно тоньше красные кольца и меньший blur.  
  ✅ *Фикс:* уменьшены коэффициенты `outerRedWidthK`/`innerRedWidthK`, shadowBlur снижен с 6 до 5; дополнительно откалиброван контраст, визуально совпадает с эталоном.
...
- ✅ `components/counters/RouteWreath.tsx`: создан общий блок для повторного использования CounterWreath на маршрутах AUDIT/CONTENT/NEWS, добавлены описания и aria-обёртки.
- ✅ `styles/counter-wreath.css`: фоновые градиенты и псевдоэлементы убраны; визуал теперь полностью рендерит canvas.
...
**Оставшиеся задачи**
- ⚠️ Провести профилирование FPS и подтвердить 60 fps при волне/реверсе.
- ⚠️ Проверить поведение prefers-reduced-motion на реальных устройствах и задокументировать результат.
...
- ⚠️ *Расхождение:* плитки в референсе тонко подсвечены и кольца тоньше.  
  ✅ *Исправление:* скорректированы коэффициенты `outerRedWidthK/innerRedWidthK`, убран лишний blur и усилен контраст — визуально совпало с HTML-референсом.
