...
* [ ] Старые круги и «ореолы» убраны / не влияют на венки.
* [ ] `CounterWreath` подключён на HOME, сетка ровная, без наложений.
* [ ] Два красных кольца читаются; плитки поверх, с редким наслаиванием.
* [ ] Плавные анимации на hover/click; реверс на mouseleave; без фонового rAF.
* [ ] Нет TS-ошибок/варнингов в `wreath.ts`.
* [ ] A11y: `role`, `aria-label`, `aria-live`, клавиатура.
* [ ] 60 fps в профайлере, `prefers-reduced-motion` соблюдён.
* [ ] Компонент легко переиспользуется на маршрутах AUDIT/CONTENT/NEWS.
