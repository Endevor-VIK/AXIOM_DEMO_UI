<!--
AXS_HEADER_META:
  id: AXS.AXUI.OPS_AGENT_OPS_README_MD
  title: "AXIOM AgentOps — Log & Commit System"
  status: ACTIVE
  mode: Doc
  goal: "Document"
...
Единый стандарт для AI-агента: фиксируй каждое действие в GLOBAL LOG, веди индекс, клади TASK LOG LINK рядом со SPEC, коммить маленькими шагами. Референс реального лога: `ops/agent_ops/logs/0002_tunnel-dev-implementation.md`.
...
- Картотека: `ops/agent_ops/logs/00_LOG_INDEX.md`, `ops/agent_ops/logs/NNNN_<task_slug>.md`
- Шаблоны: `ops/agent_ops/templates/LOG_TEMPLATE.md`, `ops/agent_ops/templates/TASK_LOG_LINK_TEMPLATE.md`
- Внутри задачи: SPEC + `<SPEC_BASENAME>_LOG_LINK.md` рядом (если есть SPEC)

## Нумерация GLOBAL LOG (NNNN)
- Имена: `ops/agent_ops/logs/NNNN_<task_slug>.md`, где NNNN — 0001, 0002, …
- Найди максимум по `^\d{4}_` в `ops/agent_ops/logs/` → новый ID = max + 1; если пусто — `0001`.
...
2) Создай GLOBAL LOG: скопируй шаблон в `ops/agent_ops/logs/NNNN_<task_slug>.md`, заполни шапку (ISO время, агент/модель, репо, ветка, задача, SPEC, статус=ACTIVE).  
3) Запиши стартовое действие в Step A.  
4) Обнови индекс `ops/agent_ops/logs/00_LOG_INDEX.md` (ID, Date/Start, Task, Branch, SPEC, GLOBAL LOG, Status).  
...
- [ ] Есть `ops/agent_ops/logs/00_LOG_INDEX.md`
