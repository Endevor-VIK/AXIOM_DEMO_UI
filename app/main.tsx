...
// Routes (will be implemented in subsequent canvases)
...
const BootRedirect = () => <TerminalBoot />;

const router = createBrowserRouter([
  {
    path: '/',
    element: <BootRedirect />,
  },
  {
    path: '/login',
    element: <LoginPage />,
...
function mount() {
  ...
  if (!el) {
    const msg = '[AXIOM] Missing #root element';
    console.error(msg);
    const fallback = document.createElement('div');
    fallback.id = 'root';
    document.body.appendChild(fallback);
    return createRoot(fallback).render(<RouterProvider router={router} />);
  }
